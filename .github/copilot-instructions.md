...
│   ├── item.py      # 数据模型 (TaskItem, ConfigItem)
│   ├── events.py    # Core 事件总线 (CoreSignalBus, FromServiceCoordinator)
...
|----------|----------|--------|--------------|--------------|
| `CoreSignalBus` | `app/core/events.py` | Core↔Service 内部通信 | Service / Runner | Service / ServiceCoordinator |
| `FromServiceCoordinator` | `app/core/events.py` | Core→View 单向通知 | ServiceCoordinator / Runner | View 层 |
...
- **`app/core/runner/` 和 `app/core/service/` 中的代码禁止直接 import `signalBus`（全局信号总线）。**
   - 历史上存在此类违规；当前治理目标是保持 `app/core/` 不再回退到对全局 UI 总线的直接依赖。
   - 新增的 Core 层通信必须通过 `CoreSignalBus` 或 `FromServiceCoordinator`。
- View 层通过 `FromServiceCoordinator` 信号或 `signalBus` 全局信号响应 Core 层事件，**不要在 View 中直接调用 Service 的内部方法改变业务状态**。
...
#### Core 事件总线（`app/core/events.py`）
- `CoreSignalBus` 只承载 Core 内部状态变化事件，不承载命令型请求。
- `FromServiceCoordinator` 只承载 Core→View 单向通知。
...
- 监听 `FromServiceCoordinator` / `signalBus` 获取状态更新。
...
用户操作 → View → ServiceCoordinator.method() → Service → CoreSignalBus
                                                              ↓
View ← signalBus / FromServiceCoordinator ← ServiceCoordinator ← CoreSignalBus
...
1. **`signalBus` 在 Core 层的使用**
   - 历史上曾存在 Core 直接触达全局 UI 总线的问题。
   - 当前约束：新功能必须通过 Runner 自身的 `Signal` → `ServiceCoordinator` 中继，不得回退。

2. **View 直接修改 Model 属性**
   - 部分 View 仍直接操作 `task.task_option`、`task.is_hidden` 等字段。
