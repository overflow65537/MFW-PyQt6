...
def _resolve_maafw_dir(install_root: Path) -> Path:
    """MaaFW 原生库目录：默认发行根下的 ./maafw。

    macOS 因 dylib rpath 为 ``@loader_path/../../``，打包后使用
    ``maafw/maa/bin``（主库）+ ``maafw/``（配件）。
    """
    if sys.platform == "darwin":
        nested = install_root / "maafw" / "maa" / "bin"
        if (nested / "libMaaFramework.dylib").is_file():
...
# 打包版：MaaFramework 等原生库放在发行根下的 maafw/（macOS 为 maafw/maa/bin）
if is_packed():
