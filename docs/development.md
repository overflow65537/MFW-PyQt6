...
Windows、Linux 与 macOS 均由 `.github/workflows/install.yml` 中的 Nuitka job 构建。macOS 主程序使用 `app-dist`，更新器使用 `onefile`。macOS 还需 `imageio`（见 `requirements-nuitka.txt`）以将 PNG 图标转为 `.icns`：
