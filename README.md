...
- 完整支持 [interface v2 协议](https://github.com/MaaXYZ/MaaFramework/blob/main/docs/zh_cn/3.3-ProjectInterfaceV2%E5%8D%8F%E8%AE%AE.md) V2.9.1
...
- `--reuse-existing`：优先复用已有实例；单独使用不会运行任务，只有同时指定 `--direct-run` 才会执行（已有实例忙时跳过；再与 `--force-restart` 同时使用时先停止当前任务再运行，不关闭进程）
- `--dev`：启用调试模式（显示测试页）
