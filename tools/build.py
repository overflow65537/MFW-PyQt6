...
for f in os.listdir(bin_dir):
    ...
    bin_files.append(f)
    dest_dir = "maa/bin" if sys.platform == "darwin" else "."
...
if sys.platform == "darwin":
    for item in Path(maa_path).parent.iterdir():
        ...
        if not (name.endswith(".dylib") or ".so." in name or name.endswith(".so")):
            continue
...
if sys.platform == "darwin":
    # 保留 pip 两层布局以满足 @loader_path/../../：
    #   maafw/maa/bin/*.dylib  主库
    #   maafw/libfastdeploy_*.dylib  配件
    ...
    for item in Path(internal_dir).iterdir():
        name = item.name.lower()
        if item.is_file() and any(name.startswith(p) for p in _maa_dep_prefixes):
