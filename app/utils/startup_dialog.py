    ...
    def _setup_ui(self):
        ...
        if self.config.buttons:
            ...
            for btn_config in self.config.buttons:
                ...
                # 绑定点击事件
                btn.clicked.connect(lambda checked, b=btn_config: self._on_button_clicked(b))
...
def _tr(text: str) -> str:
    """翻译辅助函数"""
    return QCoreApplication.translate("StartupDialog", text)
...
def show_vcredist_missing_dialog(parent=None) -> None:
    ...
    download_url, platform_name = get_vcredist_download_url()

    config = StartupDialogConfig(
        dialog_type=StartupDialogType.CRITICAL,
        title=_tr("Missing Runtime Library"),
        content=_tr(
...
def show_instance_running_dialog(parent=None) -> None:
    ...
    config = StartupDialogConfig(
        ...
        title=_tr("Program Already Running"),
        content=_tr(
...
def show_uncaught_exception_dialog(
    ...
    config = StartupDialogConfig(
        ...
        buttons=[
            DialogButton(
                text=_tr("Copy Error Info"),
                is_primary=False,
                callback=lambda: _copy_to_clipboard(tb_text),
            ),
            DialogButton(
                text=_tr("Continue"),
                is_primary=True,
...
def _ensure_app_exists() -> QApplication:
