...
def _enum_to_dict(enum_class, exclude: set[str] | None = None) -> Dict[str, int]:
    """从枚举类生成字典映射表，排除指定成员"""
    if exclude is None:
        exclude = {"Null", "All", "Default"}
    return {
        name: member.value
...
class ResourceSettingMixin:
    ...
    # 映射表定义（从枚举生成）
    WIN32_INPUT_METHOD_ALIAS_VALUES: Dict[str, int] = _enum_to_dict(
        MaaWin32InputMethodEnum
    )
    WIN32_SCREENCAP_METHOD_ALIAS_VALUES: Dict[str, int] = _enum_to_dict(
        MaaWin32ScreencapMethodEnum
    )
    ADB_SCREENCAP_OPTIONS: Dict[str, int] = {
        "default": -1,
        **_enum_to_dict(MaaAdbScreencapMethodEnum),
    }
    ADB_INPUT_OPTIONS: Dict[str, int] = {
        "default": -1,
        **_enum_to_dict(MaaAdbInputMethodEnum),
    }
    ...
    def _init_controller_defaults_from_interface(self):
        """从interface中读取controller的默认参数并保存到配置中（仅当配置为空时）"""
        interface = self.service_coordinator.interface
        controllers = interface.get("controller", [])
        has_new_defaults = False
        ...
        for controller in controllers:
            ...
            controller_type = controller.get("type", "").lower()
            controller_cfg = self.current_config.get(controller_name)

            # 如果配置不存在或为空，从interface中读取默认参数
            if not controller_cfg or not isinstance(controller_cfg, dict):
                controller_cfg = {}
                self.current_config[controller_name] = controller_cfg

            # 对于Win32控制器，从win32字段读取默认参数
            if controller_type == "win32":
                ...
                if isinstance(win32_config, dict):
                    ...
                    # 读取鼠标输入方法
                    if "mouse_input_methods" not in controller_cfg or not controller_cfg.get("mouse_input_methods"):
                        mouse_value = self._find_win32_candidate_value(
                            normalized, ["mouse_input", "mouse"], "mouse"
                        )
                        if mouse_value is not None:
                            controller_cfg["mouse_input_methods"] = mouse_value
                            has_new_defaults = True

                    # 读取键盘输入方法
                    if "keyboard_input_methods" not in controller_cfg or not controller_cfg.get("keyboard_input_methods"):
                        keyboard_value = self._find_win32_candidate_value(
                            normalized, ["keyboard_input", "keyboard"], "keyboard"
                        )
                        if keyboard_value is not None:
                            controller_cfg["keyboard_input_methods"] = keyboard_value
                            has_new_defaults = True

                    # 读取截图方法
                    if "win32_screencap_methods" not in controller_cfg or not controller_cfg.get("win32_screencap_methods"):
                        screencap_value = self._find_win32_candidate_value(
                            normalized,
                            ["screencap", "screencap_method", "screencap_methods", "screenshot", "screen_cap", "screen"],
                            "screencap",
                        )
                        if screencap_value is not None:
                            controller_cfg["win32_screencap_methods"] = screencap_value
                            has_new_defaults = True

            # 对于ADB控制器，从adb字段读取默认参数
            elif controller_type == "adb":
                adb_config = controller.get("adb")
                if isinstance(adb_config, dict):
                    normalized = {
                        str(key).lower(): value
                        for key, value in adb_config.items()
                        if isinstance(key, str)
                    }

                    # 读取截图方法
                    if "screencap_methods" not in controller_cfg or not controller_cfg.get("screencap_methods"):
                        screencap_value = self._find_adb_candidate_value(
                            normalized,
                            ["screencap", "screencap_method", "screencap_methods", "screenshot", "screen_cap", "screen"],
                            "screencap",
                        )
                        if screencap_value is not None:
                            controller_cfg["screencap_methods"] = screencap_value
                            has_new_defaults = True

                    # 读取输入方法
                    if "input_methods" not in controller_cfg or not controller_cfg.get("input_methods"):
                        input_value = self._find_adb_candidate_value(
                            normalized,
                            ["input", "input_method", "input_methods"],
                            "input",
                        )
                        if input_value is not None:
                            controller_cfg["input_methods"] = input_value
                            has_new_defaults = True

        # 如果有新的默认参数被读取，且不在同步状态，则保存配置
        if has_new_defaults and not self._syncing:
            try:
                option_service = self.service_coordinator.option_service
                option_service.update_options(self.current_config)
                ...
                if task:
                    ...
                    option_service.task_service.update_task(task)
                logger.info("从interface读取controller默认参数并保存到配置中")
            except Exception as e:
                logger.warning(f"保存controller默认参数失败: {e}")

    def _find_adb_candidate_value(
        self,
        normalized: dict[str, Any],
        candidates: list[str],
        method_type: str | None = None,
    ) -> int | None:
        """从一组候选键中获取并转换ADB方法的整型值"""
        for candidate in candidates:
            candidate_key = candidate.lower()
            if candidate_key in normalized:
                value = normalized[candidate_key]
                # 尝试解析为整数
                int_value = self._coerce_int(value)
                if int_value is not None:
                    return int_value

                # 如果是字符串，尝试从ADB选项映射表中查找
                if isinstance(value, str):
                    normalized_value = self._normalize_method_name(value)
                    if method_type == "screencap":
                        for name, mapped_value in self.ADB_SCREENCAP_OPTIONS.items():
                            if name != "default" and self._normalize_method_name(name) == normalized_value:
                                return mapped_value
                    elif method_type == "input":
                        for name, mapped_value in self.ADB_INPUT_OPTIONS.items():
                            if name != "default" and self._normalize_method_name(name) == normalized_value:
                                return mapped_value
        return None
    ...
    def _rebuild_interface_data(self):
        ...
        # 从interface中读取controller的默认参数（仅当配置为空时）
        self._init_controller_defaults_from_interface()
