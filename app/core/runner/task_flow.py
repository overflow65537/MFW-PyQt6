...
from app.common.signal_bus import global_signal_bus, global_signal_bus
...
class TaskFlowRunner(QObject):
    ...
    def _handle_agent_info(self, info: str):
        if "| WARNING |" in info:
            ...
            global_signal_bus.log_output.emit("WARNING", info)
        elif "| ERROR |" in info:
            # 从error开始截断
            info = info.split("| ERROR |")[1]
            global_signal_bus.log_output.emit("ERROR", info)
        elif "| INFO |" in info:
            # 从info开始截断
            info = info.split("| INFO |")[1]
            global_signal_bus.log_output.emit("INFO", info)
    ...
    def _handle_maafw_custom_info(self, error_code: int):
        try:
            ...
            global_signal_bus.log_output.emit("ERROR", msg)
        except ValueError:
            logger.warning(f"Received unknown MaaFW error code: {error_code}")
            global_signal_bus.log_output.emit(
    ...
    async def run_tasks_flow(
        ...
        *,
            ...
            if not is_single_task_mode:
                ...
                for task in all_tasks:
                    ...
                    if (
                        ...
                        global_signal_bus.task_status_changed.emit(task.item_id, "waiting")
        ...
        # 连接日志输出信号
        global_signal_bus.log_output.connect(collect_log)

        # 节日彩蛋：检测当天节日并随机输出一组彩蛋文案
        await emit_holiday_startup_logs(
            lambda level, text: global_signal_bus.log_output.emit(level, text),
            lambda title: global_signal_bus.set_window_title.emit(title),
        )
        ...
        try:
            ...
            if invalid_reason is not None:
                ...
                global_signal_bus.log_output.emit("ERROR", invalid_reason)
                await self.stop_task()
                return

            # 先加载资源，再连接控制器
            logger.info("开始加载资源...")
            global_signal_bus.log_output.emit("INFO", self.tr("Starting to load resources..."))
            ...
            if self.task_service.interface.get("agent", None):
                ...
                global_signal_bus.log_output.emit("INFO", self.tr("Agent Service Start"))
            ...
            # 资源加载完成后连接控制器
            logger.info("开始连接设备...")
            global_signal_bus.log_output.emit("INFO", self.tr("Starting to connect device..."))
            ...
            if not connected:
                ...
                return
            global_signal_bus.log_output.emit("INFO", self.tr("Device connected successfully"))
            logger.info("设备连接成功")
            ...
            for task in tasks_to_run:
                ...
                # 发送任务运行中状态
                global_signal_bus.task_status_changed.emit(task.item_id, "running")
                try:
                    ...
                    if task_result == "skipped":
                        ...
                        global_signal_bus.task_status_changed.emit(task.item_id, "skipped")
                        continue
                    ...
                    if task_result is False:
                        ...
                        # 发送任务失败状态
                        global_signal_bus.task_status_changed.emit(task.item_id, "failed")
                        # 发送任务失败通知
                    ...
                    if not self._current_task_ok:
                        ...
                        global_signal_bus.task_status_changed.emit(task.item_id, status)
                        # 发送任务成功通知
                        image_bytes = await self._get_notice_screenshot_bytes()
                        send_notice(
            ...
            global_signal_bus.log_output.emit("ERROR", self.tr("Task flow error: ") + str(exc))
            import traceback
            ...
            if not self._task_flow_finished_emitted:
                ...
                try:
                    global_signal_bus.task_flow_finished.emit(
                        {
            ...
            # 断开日志收集信号
            global_signal_bus.log_output.disconnect(collect_log)
            ...
            for task in all_tasks:
                if not task.is_base_task():
                    global_signal_bus.task_status_changed.emit(task.item_id, "")
    ...
    async def stop_task(self, *, manual: bool = False):
        ...
        self._stop_task_timeout()
        if self.fs_signal_bus:
            global_signal_bus.log_output.emit("INFO", self.tr("Stopping task..."))
            self.fs_signal_bus.fs_start_button_status.emit(
                {"text": "STOP", "status": "disabled"}
            )
    ...
    def _on_task_timeout(self):
        ...
        if elapsed_hours >= 1.0:
            ...
            global_signal_bus.log_output.emit("WARNING", timeout_message)
    ...
    async def _connect_adb_controller(self, controller_raw: Dict[str, Any]):
        ...
        global_signal_bus.log_output.emit("INFO", self.tr("Auto searching ADB devices..."))
        ...
        if not adb_path:
            ...
            global_signal_bus.log_output.emit("ERROR", error_msg)
            return False
        ...
        if await self.maafw.connect_adb(
            ...
            logger.info("尝试启动模拟器")
            global_signal_bus.log_output.emit("INFO", self.tr("try to start emulator"))
            ...
            if wait_emu_start > 0:
                ...
                if await self.maafw.connect_adb(
                    ...
                    await asyncio.sleep(5)
                    return True
        global_signal_bus.log_output.emit("ERROR", self.tr("Device connection failed"))
        return False
    ...
    async def _connect_win32_controller(self, controller_raw: Dict[str, Any]):
        ...
        global_signal_bus.log_output.emit("INFO", self.tr("Auto searching Win32 windows..."))
        ...
        # 启动程序+参数，轮询搜索窗口并连接
        global_signal_bus.log_output.emit("INFO", self.tr("try to start program"))
        logger.info("尝试启动程序")
    ...
    async def _connect_gamepad_controller(self, controller_raw: Dict[str, Any]):
        ...
        global_signal_bus.log_output.emit("INFO", self.tr("Auto searching desktop windows..."))
        found_device = await self._auto_find_win32_window(
            controller_raw, controller_type, controller_name, controller_config
        )
        if found_device:
    ...
    async def _connect_playcover_controller(self, controller_raw: Dict[str, Any]):
        ...
        global_signal_bus.log_output.emit("INFO", msg)
