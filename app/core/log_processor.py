...
class CallbackLogProcessor(QObject):
    """
    回调日志处理器
    将 global_signal_bus.callback 信号转换为 global_signal_bus.log_output 信号
    在 core 层统一处理日志输出逻辑
    """
    ...
    def __init__(self, parent=None):
        ...
        # 连接 callback 信号到处理函数
        global_signal_bus.callback.connect(self._on_callback)
    ...
    def _on_callback(self, signal: dict):
        ...
        if signal_name == "speed_test":
            ...
            global_signal_bus.log_output.emit(level, message)
            return
    ...
    def _handle_resource_signal(self, status: int):
        ...
        if status == 3:
            ...
            global_signal_bus.log_output.emit("ERROR", message)
    ...
    def _handle_controller_signal(self, status: int):
        ...
        if status == 1:
            ...
            global_signal_bus.log_output.emit("INFO", message)
        elif status == 3:
    ...
    def _dispatch_display(self, message: str, channels: list):
        ...
        for channel in channels:
            ...
            if channel == "log":
                ...
                global_signal_bus.focus_toast.emit(message)
            elif channel == "notification":
                global_signal_bus.focus_notification.emit(message)
            elif channel == "dialog":
                global_signal_bus.focus_dialog.emit(message)
            elif channel == "modal":
                global_signal_bus.focus_modal.emit(message)
            else:
                # 未知渠道，默认走日志
                global_signal_bus.log_output.emit("INFO", message)
